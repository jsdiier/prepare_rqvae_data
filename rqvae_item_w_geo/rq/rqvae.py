...
from datasets import EmbDataset, StreamingEmbBatchDataset
...
def parse_args():
    ...
    parser.add_argument("--streaming", action="store_true", help="stream embeddings by batch instead of loading all into memory")
    parser.add_argument("--no_streaming", action="store_true", help="disable streaming mode and load all embeddings into memory")
...
if __name__ == '__main__':
    ...
    use_streaming = args.streaming or not args.no_streaming
    logging.info("Effective data loading mode: %s", "streaming" if use_streaming else "in_memory")

    """build dataset"""
    if use_streaming:
        data = StreamingEmbBatchDataset(args.data_path, batch_size=args.batch_size)
        if args.num_workers != 0:
            logging.warning("Streaming mode requires num_workers=0, overriding the configured value %s", args.num_workers)
